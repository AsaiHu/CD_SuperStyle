...
var $iconElement = $("#" + CTRL_PREFIX + "ImageIcon");
    var $imagedrag = $("#" + CTRL_PREFIX + "ImageDrag");
    ...
    //初始化默认值
    $("#" + CTRL_PREFIX + "txt_width").val(currentWidth);
    $("#" + CTRL_PREFIX + "txt_height").val(currentHeight);
    $("#" + CTRL_PREFIX + "txt_top").val(0 - dragtop);
    $("#" + CTRL_PREFIX + "txt_left").val(0 - dragleft);
    $("#" + CTRL_PREFIX + "txt_Zoom").val(scaleFactor);
    ...
    $imagedrag.draggable(
    {
        ...
        drag: function(e, ui) {
            ...
            var drop_img = $("#" + CTRL_PREFIX + "ImageIcon");
            ...
            $("#" + CTRL_PREFIX + "txt_left").val(0 - parseInt(self.position.left));
            $("#" + CTRL_PREFIX + "txt_top").val(0 - parseInt(self.position.top));
        }
    }
    );
    //设置图片可拖拽，并且拖拽一张图片时，同时移动另外一张图片
    $iconElement.draggable(
    {
        ...
        drag: function(e, ui) {
            var self = $(this).data("draggable");
            var drop_img = $("#" + CTRL_PREFIX + "ImageDrag");
            var top = drop_img.css("top").replace(/px/, ""); //取出截取框到顶部的距离
            var left = drop_img.css("left").replace(/px/, ""); //取出截取框到左边的距离
            drop_img.css({ left: (parseInt(self.position.left) - LEFT_EDGE) + "px", top: (parseInt(self.position.top) - TOP_EDGE) + "px" }); //同时移动内部的图片
            $("#" + CTRL_PREFIX + "txt_left").val(0 - (parseInt(self.position.left) - LEFT_EDGE));
            $("#" + CTRL_PREFIX + "txt_top").val(0 - (parseInt(self.position.top) - TOP_EDGE));
        }
